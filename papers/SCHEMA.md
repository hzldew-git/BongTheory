...
manifest is committed. A paper may be held out of GitHub artifacts and
releases only by recording `deployment.githubReviewKit: false` in its own
manifest; handwritten workflow exclusion lists are forbidden. The field
defaults to `true` when absent. Local Review Kit generation remains available
for a deferred paper, and the deferral does not change its mathematical
coverage status.
...
The optional `deployment` object records distribution policy rather than
proof status. Set `githubReviewKit` to `false` only for an explicit temporary
