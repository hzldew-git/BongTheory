...
| He classic Section 8 and Theorems 1.7--1.9 | Lemma 8.1, Proposition 8.2, even Lemma 8.3/Theorems 1.7--1.8, and the remaining global deductions | conditional source logic proved from explicit number-field, extension, and strong-approximation packages; canonical height-one-spectrum arithmetic derives the discriminant laws and Section 8 arithmetic fields from a place equivalence; all three Lemma 8.1 clauses are proved on the actual selected finite completions at numerical/coefficient-criterion scope, including order and defect scaling plus good-BONG coefficient transfer; the global lattice/localization and orthogonal-basis bridge remain explicit; no unrestricted odd Lemma 8.3/Theorem 1.7/Theorem 1.8 endpoint is exported, while Theorem 1.7's common logical tail has its missing local-defect premise explicit; Classic Reports 23--41 |
