...
lower-even J2 condition. All residue-cardinality formulas are now
unconditional: O'Meara 63:9 follows from the principal-unit filtration and an
explicit finite equivalence, while O'Meara 63:5 follows by restricting that
equivalence to the depth-two subgroup and partitioning the finite index.
...
The field-law interfaces have generic proved implementations, and both unit
square-class counting formulas are proved internally. No `sorry`, `admit`, new
axiom declaration, or native evaluation was found in the inspected Classic
files. These checks do not replace an inspection of the actual theorem
premises.
