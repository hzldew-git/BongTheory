...
| Lemma 8.1, Proposition 8.2, Lemma 8.3, Theorems 1.7--1.9 | MIXED: all three clauses of Lemma 8.1 are proved on the actual selected finite completions at numerical/coefficient-criterion scope, including order scaling, relative-defect scaling, and good-BONG coefficient transfer; the global-lattice localization and orthogonal-basis identification remain pending. Proposition 8.2 and both sufficiency stages of Theorem 1.9 are derived from lower laws rather than assumed as final fields; the number-field discriminant theorem and canonical global arithmetic adapter are concrete. Lemma 8.3, Theorem 1.7, and Theorem 1.8 are exposed only for `n >= 2` even, with no unrestricted odd endpoint; Theorem 1.7's parity-independent final contradiction is separately proved from an explicit local-defect premise; remaining number-field lattice, local sum-of-squares, and strong-approximation instances are pending; Reports 23--41 |
...
| Exact clean-kit verification | Historical PASS locally at `c1ee018`: 2,010 payload hashes, resumed fresh-extraction 5,671-job build, seven audit/gate checks, and 62,746-declaration axiom gate; Report 35. The current `c6d22da` code-and-audit checkpoint is pending its own fresh kit |
