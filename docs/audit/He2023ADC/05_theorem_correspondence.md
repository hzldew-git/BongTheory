...
| Theorem 6.2 | `Bong.Lattice.heADC2025Theorem62_of_four_le`; `Bong.BONG.GoodBONG.not_heADC2025Theorem62BinaryStatement` | `FULLY_FORMALIZED` / `PROVISIONAL_MATCH` for `n>=4`; `SEMANTIC_MISMATCH` at `n=2`, where the exact printed biconditional is formally refuted by an actual nonmaximal 2-ADC lattice not isometric to the listed exception; report 34 |
...
| Section 5, Theorem 7.2, Remark 7.3, Lemmas 7.14--7.15, Definition 7.16, Remark 7.17, Lemmas 7.18--7.20, Corollary 7.21, Section 8 and remaining Section 1 results | none | Pending; every downstream use of the unqualified Theorem 6.2 requires a separate boundary audit |
