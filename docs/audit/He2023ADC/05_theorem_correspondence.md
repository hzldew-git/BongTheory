...
| Lemma 7.11 | `heADC2025Lemma711Odd`, `heADC2025Lemma711_badBranch_impossible` | `SPECIAL_CASE_ONLY`: all odd-valuation `N_2^3(delta*pi)` tests and the exact existence contradiction needed by Lemma 7.5(iii) are proved. The source quantifies over every nonzero parameter, so its even-valuation branch remains pending; report 36 |
