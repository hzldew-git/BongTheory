# Executive summary

The project now has a publisher-locked, machine-checkable statement target for
He--Hu Theorem 1.1, direct checked endpoints for every numbered result in
Section 2 and for the Section 3 core through Proposition 3.7 together with
Lemmas 3.9--3.11 and 3.13--3.14, and a proved general maximal-testing
reduction. In particular, the complete
quadratic-space classification, unique excluding-space theorem, and every
Table 2 maximality row in Proposition 3.7 are now kernel checked. The
Section 4 condition layer and the full arithmetic conversion from
`I1^E`--`I3^E` to the concise Theorem 4.7 invariants are also kernel checked,
as are Lemma 4.2 in both directions, the complete exceptional-target
Lemma 4.3, and the full Lemmas 4.4--4.5 equivalences for all integral even-rank
targets.  The direct Theorem 4.1 endpoint now assembles Lemmas 4.2, 4.4, and
4.5 through the checked Theorem 2.8 factorization. The principal mathematical
Section 5 foundation is also checked through Remark 5.2 and Lemmas 5.4,
5.6--5.9, including the two-test Hilbert-symbol obstruction with its exact
at-least-one quantifier. The principal mathematical work remains Lemmas 5.10--5.11,
the Section 5 equivalence and theorem endpoints, Section 6, the two main
theorems, and the explicit minimal testing set.
Status is partial, not a verified match.
