...
| Proposition 2.5 | `heHu2022Proposition25` | Proved; both clauses |
| Proposition 2.6 | `heHu2022Proposition26` | Proved; all seven clauses |
| Proposition 2.7 | `heHu2022Proposition27i`, `heHu2022Proposition27ii`, `heHu2022Proposition27iiiiv`, `heHu2022Proposition27v` | Proved; all five clauses |
| Theorem 2.8 | `heHu2022Theorem28` | Proved through the unconditional Beli representation theorem |
...
| Lemma 5.9 | `heHu2022Lemma59i`, `heHu2022Lemma59ii`, `heHu2022Lemma59` | Proved with the literal `N_1^n(c)` and `N_1^n(c*cTilde#)` good BONGs. Both tests satisfy the two strict inequalities in clause (i). Clause (ii) is retained with its published simultaneous quantifier: the source cannot represent both tests, so condition (iii') fails for at least one; the formalization does not strengthen this to separate nonrepresentation of each test. |
| Lemma 5.10 | `HeHuLemma510TestConditions`, `heHu2022Lemma510` | Proved in all three directions. Condition (ii) preserves both first-column tests under one sharp witness and the existential valuation-unit parameter in the exceptional ternary test. In `(iii) -> (i)`, nonterminal triggers are transported to the even prefix and Lemma 4.4; the terminal `alpha_n=0` case uses the exact `W_1` normal forms and Lemma 3.14(ii), while `alpha_n=1` follows the published Lemma 2.11, equation (5.5), and `G_n` parity contradiction. The ambient `n`-universality premise is retained in the public equivalence. |
| Lemma 5.11 | `HeHuLemma511TestConditions`, `heHu2022Lemma511` | Proved in all three directions. Condition (ii) is the simultaneous pair `N_1^n(c)`, `N_2^n(c)` with the normalized representative of the printed square class. The second literal test is built from the two Lemma 3.11(ii) ternary rows and lifted by Lemma 3.10. Under a terminal gap greater than `2e`, `I2^E` and `I2^O` force the intermediate orders and test parity, after which Lemma 3.13 forbids both tests from representing the same codimension-two source prefix. Nonterminal sufficiency descends to Lemma 4.5; the terminal case contradicts `I3^O`. The ambient premise is retained. |
| Proposition 5.5 | `heHu2022Proposition55` | Proved in both directions. Necessity first proves the paper's implicit `n`-universal to `(n-1)`-universal descent by adjoining the standard integral unit line, then invokes Theorem 4.1 and specializes Lemmas 5.10--5.11. Sufficiency assembles all four Theorem 2.8 components using Lemma 5.6 and the universal directions of Lemmas 5.10--5.11. |
| Theorem 5.1 | `heHu2022Theorem51Odd` | Proved for every odd `n=2k+3`. The forward comparison extracts `I1^O` from `I1^E` and `I2^E`; the reverse comparison uses both alpha cases of Lemma 5.4 to reconstruct `I2^E` and `I3^E`, then applies Proposition 5.5. |
| Theorem 5.3 | `HeHuTheorem53Conditions`, `heHu2022Theorem53Odd` | Proved as an exact rephrasing of Theorem 5.1. The two parenthetical equivalences in clause (i) are separate checked endpoints derived from Proposition 2.6(i), the alternating initial profile, Proposition 2.7(i), and integrality. |
| Lemma 6.1 | `heHu2022Lemma61_equation61`, `heHu2022Lemma61i`, `heHu2022Lemma61ii`, `heHu2022Lemma61` | Proved with both clauses. Clause (i) proves equation (6.1) and expands the capped defect as the minimum of the raw defect and its two alpha caps. Clause (ii) proves the exact tail-candidate existential, including the automatic `R_(n+2)=-2e` and `R_(n+2)=1` cases. |
