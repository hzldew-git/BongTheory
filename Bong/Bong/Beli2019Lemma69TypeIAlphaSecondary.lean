...
/-!
# Beli (2019), Lemma 6.9(ii): type-I source-alpha secondary candidate

For a noninitial even boundary, two adjacent source defects and the earlier
same-parity diagonal defect pass through two applications of the domination
principle.  The type-I order shifts cancel in the secondary coefficient.
-/
...
-- Two nested domination cuts carry six dependent prefix indices; the first
-- central even boundary also normalizes an empty diagonal prefix.
/-- The source alpha is below the secondary candidate at every interior even
central type-I boundary, including `i = 2`. -/
theorem lemma69_typeI_alpha_le_secondary_of_earlier
  ...
  let previousTwo : Fin (n + 1) :=
    ⟨i.val - 3, hiEarlierAlpha⟩
  rcases heven with ⟨d, hd⟩
  have hearlierEven : Even (i.val - 2) := ⟨d - 1, by omega⟩
  have hpreviousOdd : Odd (i.val - 1) := ⟨d - 1, by omega⟩
  have hearlierGap := lemma69_v_typeI_even_entry_gap_two
    a b D C hfirst (i.val - 2) hearlierEven hleft (by omega)
  have hpreviousGap := lemma69_v_typeI_odd_entry_gap_two
  ...
  have hrightGoodRaw := a.good
    (⟨i.val - 2, by omega⟩ : Fin (n + 2)) (by
      ...
      omega)
  have hrightGood : a.order ⟨i.val - 2, hiEarlier⟩ ≤
      a.order ⟨i.val, i.lt_large⟩ := by
    have hindex :
        (⟨(⟨i.val - 2, hiEarlier⟩ : Fin (n + 2)).val + 2,
          by
            change (i.val - 2) + 2 < n + 2
  ...
  have hrightShift :
    ...
    dsimp only [coefficient]
    push_cast
    have hgoodQ : (a.order ⟨i.val - 2, hiEarlier⟩ : ℚ) ≤
        a.order ⟨i.val, i.lt_large⟩ := by
  ...
  have hdiagonalBound : (a.alphaValue p : WithTop ℚ) ≤
    ...
    · have hthree : 2 < i.val := by
      ...
      let earlierIdx : RepresentationIndex (n + 2) (n + 2) :=
        ⟨i.val - 2, by omega, hiEarlier, by omega⟩
  ...
  have htargetEarlier : b.order ⟨i.val - 2, hiEarlier⟩ =
      a.order ⟨i.val - 2, hiEarlier⟩ + 2 := by
    rw [← b.orderSequence_entryOrZero_eq_order
        ⟨i.val - 2, hiEarlier⟩,
      ← a.orderSequence_entryOrZero_eq_order
        ⟨i.val - 2, hiEarlier⟩]
    simpa only using hearlierGap
