...
import Bong.Bong.Beli2019Lemma912RankFourLemma99
import Bong.Bong.Beli2019Lemma912ScalarConditions
import Bong.Bong.Beli2019Lemma912AnisotropicAssembly

/-!
# Beli (2019), Lemma 9.12: quaternary type-I scalar inequalities

The first scalar inequality is proved directly at rank four.  The remaining
representation indices use the existing arbitrary-tail estimates.  The
anisotropic case is an instance of the general scalar-failure propagation
argument with tail length one.
-/
...
/-- The signed first-three/first-one product has even order in the
quaternary residual profile. -/
theorem fullRawOrder_even_rankFour
    (a : GoodBONG q M 4) (c : GoodBONG s Q 4)
  ...
  have horder : ordUnit K
      ((-1 : Kˣ) * a.prefixProduct 3 * c.prefixProduct 1) =
        a.order (0 : Fin 4) + a.order (1 : Fin 4) +
          a.order (2 : Fin 4) + c.order (0 : Fin 4) := by
...
private theorem intCast_add_two_le_alphaValue_of_odd_of_lt_of_halfGap_le_rankFour'
    ...
    (c : GoodBONG s Q 4) (i : Fin 3) (z₀ : Int)
...
-- The discrete defect jump and both alpha-cap estimates elaborate together.
/-- First scalar inequality in the quaternary below-half-gap branch. -/
theorem firstScalar_of_belowHalfGap_rankFour
    ...
    (a : GoodBONG q M 4) (c : GoodBONG s Q 4)
    (profile : Beli2019Lemma912InitialProfileAllRanks (T := 1) a c)
    (data : Beli2019Lemma912TypeIBetaDataRankFour a c A₁ (A₁ + 2))
    (hfirst : a.order (0 : Fin 4) = c.order (0 : Fin 4))
  ...
  have hAodd : Odd A₁ := by
    rcases hAoddRational with ⟨t, htOdd, ht⟩
    have hAt : A₁ = t := by exact_mod_cast data.firstAlpha.symm.trans ht
    simpa only [hAt] using htOdd
  have hgapSharp :=
    a.beli2019Lemma912_firstGap_le_twoE_sub_four_of_below_halfGap_rankFour
      c profile hbelow
  ...
  have hHBoundQ : (H : ℚ) ≤ 2 * (ramificationIndex K : ℚ) - 2 := by
    have hgapQ : (a.orderGap (0 : Fin 3) : ℚ) ≤
  ...
  have hxEven : Even (ordUnit K x) := by
    simpa only [x] using fullRawOrder_even_rankFour a c profile hfirst
  ...
  have hleft : ((((A₁ + 2 : Int) : ℚ) : WithTop ℚ) ≤
    ...
    exact WithTop.coe_le_coe.mpr data.betaThird
  have hrightStrict : (A₁ : ℚ) < c.alphaValue (0 : Fin 3) := by
  ...
  have hhalfShift : a.halfGapValue (0 : Fin 3) + 1 ≤
    ...
    have hcast : (0 : Fin 3).castSucc = (0 : Fin 4) := by rfl
    rw [hsucc, hcast]
    have horderQ : (a.order (1 : Fin 4) : ℚ) + 2 ≤
        (c.order (1 : Fin 4) : ℚ) := by exact_mod_cast data.sourceSecondOrder
    have hfirstQ : (a.order (0 : Fin 4) : ℚ) =
        (c.order (0 : Fin 4) : ℚ) := by exact_mod_cast hfirst
    push_cast
    linarith
  ...
  have hrightQ : ((A₁ + 2 : Int) : ℚ) ≤ c.alphaValue (0 : Fin 3) := by
    letI : Beli2006AlphaLaws.{u, z} K := targetLaws
    letI : Beli2009AlphaParityLaws.{u, z} K := targetParity
    exact intCast_add_two_le_alphaValue_of_odd_of_lt_of_halfGap_le_rankFour'
      c (0 : Fin 3) A₁ hAodd hrightStrict hrightHalf
...
/-- First scalar inequality at an isotropic quaternary half-gap. -/
theorem firstScalar_of_halfGapIsotropic_rankFour
    [targetLaws : Beli2006AlphaLaws.{u, z} K]
    [targetParity : Beli2009AlphaParityLaws.{u, z} K]
    {A₁ : Int}
    (a : GoodBONG q M 4) (c : GoodBONG s Q 4)
    (data : Beli2019Lemma912TypeIBetaDataRankFour a c A₁ (A₁ + 1))
    (hstrict : (a.alphaValue (0 : Fin 3) : WithTop ℚ) <
      a.truncatedPrefixDefect c (-1) 3 1)
  ...
  have hrightStrict : (A₁ : ℚ) < c.alphaValue (0 : Fin 3) := by
    ...
    rw [data.firstAlpha,
      c.prefixAlphaCap_of_internal (by omega) (by omega)] at htop
    exact WithTop.coe_lt_coe.mp htop
  have hrightQ : ((A₁ + 1 : Int) : ℚ) ≤ c.alphaValue (0 : Fin 3) := by
    letI : Beli2006AlphaLaws.{u, z} K := targetLaws
    letI : Beli2009AlphaParityLaws.{u, z} K := targetParity
    exact c.intCast_add_one_le_alphaValue_of_lt_of_le_twoE
      (0 : Fin 3) A₁ hbetaTwoE hrightStrict
...
/-- The Lemma 9.6 exclusion removes the anisotropic boundary `2e - 2` at
rank four. -/
theorem firstGap_le_twoE_sub_four_of_strict_anisotropic_rankFour
    [sourceLaws : Beli2006AlphaLaws.{u, v} K]
    [sourceParity : Beli2009AlphaParityLaws.{u, v} K]
    [targetLaws : Beli2006AlphaLaws.{u, z} K]
    [targetParity : Beli2009AlphaParityLaws.{u, z} K]
    ...
    (a : GoodBONG q M 4) (c : GoodBONG s Q 4)
    (profile : Beli2019Lemma912InitialProfileAllRanks (T := 1) a c)
    (data : Beli2019Lemma912TypeIBetaDataRankFour a c A₁ A₁)
    (hstrict : (a.alphaValue (0 : Fin 3) : WithTop ℚ) <
      a.truncatedPrefixDefect c (-1) 3 1)
  ...
  have hne : a.orderGap (0 : Fin 3) ≠
    ...
    have hthird : ((A₁ + 1 : Int) : ℚ) ≤ a.alphaValue (2 : Fin 3) := by
      apply a.intCast_add_one_le_alphaValue_of_lt_of_le_twoE
        (2 : Fin 3) A₁ (by omega)
      simpa only [← data.firstAlpha] using hthirdStrict
    let shiftedData : Beli2019Lemma912TypeIBetaDataRankFour
        a c A₁ (A₁ + 1) := {
      firstAlpha := data.firstAlpha
      betaLower := by omega
      betaUpper := by omega
      betaThird := hthird
      fourthOrder := data.fourthOrder
      sourceSecondOrder := data.sourceSecondOrder }
    have hscalar := firstScalar_of_halfGapIsotropic_rankFour
    ...
    have hdefect : a.Beli2019Lemma96DefectBound c := by
      ...
      rw [hcast]
      exact hscalar
    rcases profile.lemma96_exclusion_at_boundary (by omega) hgap with hnot | hnot
    · exact hnot hdefect
...
/-- Complete quaternary scalar package in the equal-alpha branch. -/
theorem typeIScalarConditions_of_equalSecondLarge_rankFour
    [sourceLaws : Beli2006AlphaLaws.{u, v} K]
    [targetLaws : Beli2006AlphaLaws.{u, z} K]
    [BONGStructuralLaws.{u, v} K]
    {R₁ R₂ A₁ : Int}
    (a : GoodBONG q M (3 + 1)) (c : GoodBONG s Q (1 + 3))
    (data : Beli2019Lemma912TypeIBetaDataRankFour a c A₁ A₁)
    (D : Beli2019Lemma99Realization (q := r) R₁ (R₂ + 2) R₁ A₁)
    (E : Beli2019Lemma910Data (N := 1) a D)
    (horders : ∀ i : Fin 3, a.order (Fin.castAdd 1 i) = ![R₁, R₂, R₁] i)
    (hlength : 3 + 1 = 1 + 3)
...
/-- Complete quaternary scalar package in the below-half-gap branch. -/
theorem typeIScalarConditions_of_belowHalfGap_rankFour
    [QuadraticDefectLaws K]
    [HilbertSymbolLaws K]
    [UnitQuadraticDefectParityLaws K]
    [sourceLaws : Beli2006AlphaLaws.{u, v} K]
  ...
  refine ⟨?_, ?_⟩
  · rw [haCast]
    exact firstScalar_of_belowHalfGap_rankFour
...
/-- Complete quaternary scalar package at an isotropic half-gap. -/
theorem typeIScalarConditions_of_halfGapIsotropic_rankFour
    [sourceLaws : Beli2006AlphaLaws.{u, v} K]
    [targetLaws : Beli2006AlphaLaws.{u, z} K]
    [targetParity : Beli2009AlphaParityLaws.{u, z} K]
    {R₁ R₂ A₁ : Int}
  ...
  have hhalfFormula : (A₁ : ℚ) =
    ...
    calc
      (A₁ : ℚ) = a.alphaValue (0 : Fin 3) := data.firstAlpha.symm
  ...
  refine ⟨?_, ?_⟩
  · rw [haCast]
    exact firstScalar_of_halfGapIsotropic_rankFour
      (targetLaws := targetLaws) (targetParity := targetParity)
...
/-- Complete quaternary scalar package at an anisotropic half-gap. -/
theorem typeIScalarConditions_of_halfGapAnisotropic_rankFour
    [QuadraticDefectLaws K]
    [HilbertSymbolLaws K]
    [UnitQuadraticDefectParityLaws K]
  ...
  have hcomparisonZero : c.order (0 : Fin 4) = R₁ :=
    hfirst.symm.trans hR₁
  have hcomparisonOne : R₂ + 2 ≤ c.order (1 : Fin 4) := by
    simpa only [hR₂] using data.sourceSecondOrder
  have haCast : a.castLength hlength = a := by
    cases hlength
    rfl
  ...
  have hthirdStrict : (A₁ : ℚ) <
      (a.castLength hlength).alphaValue (2 : Fin 3) := by
    rw [haCast]
    simpa only [← data.firstAlpha] using
      a.beli2019Lemma912_firstAlpha_lt_thirdAlpha_of_fullDefect_strict_rankFour
        c hstrict
