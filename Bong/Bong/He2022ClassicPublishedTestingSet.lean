...
unit square class. O'Meara 63:5 and 63:9 are both derived from the proved
principal-unit filtration in `Bong.Dyadic.UnitSquareClassCount`; no
paper-specific counting premise is assumed.
-/
...
/-- The unit square class selected by one entry of the published complete
representative system.  This is kept in the classic-paper development so its
review dependency graph does not pass through the later ADC paper. -/
noncomputable def heClassicUnitRepresentativeClass
...
theorem heClassicUnitRepresentativeClass_injective
    {I : Type u} [Fintype I] (U : I -> Kˣ)
    (hU : IsHeHuCompleteUnitRepresentativeSystem (K := K) U) :
    Function.Injective (heClassicUnitRepresentativeClass U hU) := by
  intro i j hij
...
theorem heClassicUnitRepresentativeClass_surjective
    {I : Type u} [Fintype I] (U : I -> Kˣ)
    (hU : IsHeHuCompleteUnitRepresentativeSystem (K := K) U) :
    Function.Surjective (heClassicUnitRepresentativeClass U hU) := by
  intro c
...
/-- The finite published representative system is equivalent to the intrinsic
unit square-class quotient. -/
noncomputable def heClassicUnitRepresentativeEquiv
    {I : Type u} [Fintype I] (U : I -> Kˣ)
    (hU : IsHeHuCompleteUnitRepresentativeSystem (K := K) U) :
    I ≃ ValuationUnitClass K :=
  Equiv.ofBijective (heClassicUnitRepresentativeClass U hU)
    ⟨heClassicUnitRepresentativeClass_injective U hU,
      heClassicUnitRepresentativeClass_surjective U hU⟩

/-- O'Meara 63:9 transported from the intrinsic unit square-class quotient
to any complete irredundant normalized representative system. -/
theorem card_heClassicUnitRepresentatives
    {I : Type u} [Fintype I] (U : I -> Kˣ)
    (hU : IsHeHuCompleteUnitRepresentativeSystem (K := K) U) :
    Fintype.card I =
      2 * heClassicResidueNorm (K := K) ^ ramificationIndex K := by
  rw [Fintype.card_eq_nat_card]
  rw [Nat.card_congr (heClassicUnitRepresentativeEquiv U hU)]
  simpa [heClassicResidueNorm, Fintype.card_eq_nat_card] using
    card_valuationUnitClass K

private theorem heClassic_natCast_le_defectOrder_of_quadraticDefect
    (x : Kˣ) (n : Nat) (h : (n : ℕ∞) ≤ quadraticDefect K x) :
    (((n : Nat) : ℚ) : WithTop ℚ) ≤ defectOrder (K := K) x := by
  ...
  · obtain ⟨d, hd⟩ := WithTop.ne_top_iff_exists.mp htop
    ...
    unfold defectOrder
    rw [← hd]
    change (((n : Nat) : ℚ) : WithTop ℚ) ≤ ((d : ℚ) : WithTop ℚ)
    exact_mod_cast hnd

/-- A published unit representative has defect different from one exactly
when its square class belongs to the depth-two principal-unit subgroup. -/
theorem heClassicUnitRepresentativeClass_mem_two_iff
    {I : Type u} [Fintype I] (U : I -> Kˣ)
    (hU : IsHeHuCompleteUnitRepresentativeSystem (K := K) U) (i : I) :
    heClassicUnitRepresentativeClass U hU i ∈
        principalUnitValuationClassSubgroup K 2 ↔
      defectOrder (K := K) (U i) ≠ (1 : WithTop ℚ) := by
  constructor
  · intro hmem hdefect
    have hlowerRaw : (2 : ℕ∞) ≤ quadraticDefect K (U i) :=
      natCast_le_quadraticDefect_of_unitClass_mem
        ⟨U i, hU.isUnit i⟩ 2 hmem
    have hlower : (2 : WithTop ℚ) ≤ defectOrder (K := K) (U i) :=
      heClassic_natCast_le_defectOrder_of_quadraticDefect
        (K := K) (U i) 2 hlowerRaw
    rw [hdefect] at hlower
    norm_num at hlower
  · intro hne
    have hqne : quadraticDefect K (U i) ≠ (1 : ℕ∞) := by
      intro hq
      apply hne
      simpa using
        (Beli2009FinalRemarksProof.defectOrder_eq_natCast_of_quadraticDefect_eq
          (K := K) (U i) 1 hq)
    have hone : (1 : ℕ∞) ≤ quadraticDefect K (U i) :=
      one_le_quadraticDefect_of_unit (U i) (hU.isUnit i)
    have hstrict : (1 : ℕ∞) < quadraticDefect K (U i) :=
      lt_of_le_of_ne hone hqne.symm
    have htwo : (2 : ℕ∞) ≤ quadraticDefect K (U i) := by
      have hadd : (1 : ℕ∞) + 1 ≤ quadraticDefect K (U i) :=
        (ENat.add_one_le_iff (by simp : (1 : ℕ∞) ≠ ⊤)).2 hstrict
      norm_num at hadd ⊢
      exact hadd
    exact valuationUnitClassHom_mem_principalUnitValuationClassSubgroup_of_defect
      K ⟨U i, hU.isUnit i⟩ 2 htwo

/-- Restricting the preceding equivalence identifies non-defect-one published
representatives with the depth-two principal-unit square classes. -/
noncomputable def heClassicDeeperUnitRepresentativeEquiv
    {I : Type u} [Fintype I] (U : I -> Kˣ)
    (hU : IsHeHuCompleteUnitRepresentativeSystem (K := K) U) :
    {i : I // defectOrder (K := K) (U i) ≠ (1 : WithTop ℚ)} ≃
      principalUnitValuationClassSubgroup K 2 :=
  (heClassicUnitRepresentativeEquiv U hU).subtypeEquiv fun i =>
    (heClassicUnitRepresentativeClass_mem_two_iff U hU i).symm

/-- O'Meara 63:5, derived by partitioning a complete unit representative
system into defect-one and depth-two square classes. -/
theorem card_heClassicDefectOne_balance
    {I : Type u} [Fintype I] (U : I -> Kˣ)
    (hU : IsHeHuCompleteUnitRepresentativeSystem (K := K) U) :
  ...
  classical
  let p : I -> Prop := fun i =>
    defectOrder (K := K) (U i) = (1 : WithTop ℚ)
  letI : Fintype {i : I // p i} := Fintype.ofFinite _
  letI : Fintype {i : I // ¬ p i} := Fintype.ofFinite _
  have hdeeper : Fintype.card {i : I // ¬ p i} =
      Nat.card (principalUnitValuationClassSubgroup K 2) := by
    rw [Fintype.card_eq_nat_card]
    exact Nat.card_congr (heClassicDeeperUnitRepresentativeEquiv U hU)
  change Fintype.card {i : I // p i} +
      2 * heClassicResidueNorm (K := K) ^ (ramificationIndex K - 1) =
    2 * heClassicResidueNorm (K := K) ^ ramificationIndex K
  rw [← card_heClassicUnitRepresentatives (K := K) U hU]
  have hP2 : Nat.card (principalUnitValuationClassSubgroup K 2) =
      2 * heClassicResidueNorm (K := K) ^ (ramificationIndex K - 1) := by
    simpa [heClassicResidueNorm, Fintype.card_eq_nat_card] using
      card_principalUnitValuationClassSubgroup_two (K := K)
  rw [← hP2, ← hdeeper, Fintype.card_subtype_compl]
...
theorem he2022ClassicProposition28ii_even_gt_one
  ...
  rw [if_neg hene]
  have hUcard := card_heClassicUnitRepresentatives (K := K) U hU
  have hU1card := card_heClassicDefectOne_balance (K := K) U hU
  omega
...
theorem he2022ClassicProposition28ii_odd
  ...
  rw [card_heClassicPublishedOddTestingIndex,
    card_heClassicUnitRepresentatives (K := K) U hU]
  ring
