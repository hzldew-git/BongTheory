...
/-!
# Beli (2019), Lemma 6.9(ii): normalized type-II left branch

This file proves the source-alpha branch of Lemma 6.9(ii) on the alternating
left profile. Its reverse-dual form supplies the type-II right classification
needed in Lemma 7.9(ii), case 7.
-/
...
/-- Order identities at an even boundary of the normalized left type-II
profile. -/
theorem lemma69_typeII_left_boundary_orders
...
/-- Lemma 6.9(ii) on the normalized left type-II profile: at every even
boundary before the transition, the representation invariant is the source
alpha. -/
theorem beli2019Lemma69_ii_typeII_sourceLeftValue_of_center
  ...
  have hmain : ∀ (t : Nat) (htTwo : 2 ≤ t) (htLeft : t ≤ left)
    ...
    | h t ih =>
        ...
        have hpSucc : p.succ =
          ...
          omega
        have horders := lemma69_typeII_left_boundary_orders
        ...
        have htargetCurrent : b.order ⟨t - 1, by omega⟩ =
          ...
          exact horders.2.2
        have hpreviousOne :=
          a.lemma69_typeII_sourcePreviousAlpha_eq_one_of_center
...
/-- Lemma 6.9(ii) on the normalized type-II source-left branch. -/
theorem beli2019Lemma69_ii_typeII_sourceLeftValue
    [alpha : Beli2006AlphaLaws.{u, v} K]
    [Beli2009AlphaParityLaws.{u, v} K]
    (a : GoodBONG q L (n + 2)) (b : GoodBONG q M (n + 2))
    (D : Lemma67TypeII a b)
    ...
    a.representationAlphaValue b
      ...
      a.alphaValue ⟨i - 1, by
        ...
        have hlong := D.long
        omega⟩ := by
  exact a.beli2019Lemma69_ii_typeII_sourceLeftValue_of_center b D hfirst
    (a.beli2019Lemma69_i_typeII_sourceBoundary_eq_one b D)
    hdefect i hiTwo hiLeft hiEven
