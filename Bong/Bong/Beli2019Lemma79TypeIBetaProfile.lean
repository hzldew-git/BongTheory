...
/-!
# Beli (2019), Lemma 7.9(ii): the type-I beta branch from the profile

This file discharges the profile and parity premises of the target-alpha
branch in case 4.  The strict-current-order subcase combines Lemmas 6.6(i)
and 7.2(i); the other subcase is the primary-candidate estimate.
-/
...
/-- The `beta_i` branch of Lemma 7.9(ii), case 4, derived from the fixed
canonical type-I profile and the strict norm-ideal hypothesis. -/
theorem lemma79_typeI_beta_bound_from_profile
  ...
  have hiPreviousAlpha : i.val - 1 < n + 1 := by
    have hi := i.lt_large
    omega
  have hprofile := a.lemma79_typeI_target_twoStep_and_alpha
    b D C hfirst i hodd hleft hright
  ...
  · have hcurrentStrict : c.order ⟨i.val - 1, hiPrevious⟩ <
    ...
    have hfarRight : i.val + 1 ≤ C.rightSwitch := by
      rcases C.right_even with ⟨e, he⟩
      omega
    have hfarBound : i.val + 1 < n + 2 :=
      hfarRight.trans C.right_le_last |>.trans_lt
        D.profile.lastDifference.bound
    have hsourcePrevious : a.orderSequence.entryOrZero (i.val - 1) =
        a.orderSequence.entryOrZero D.anchor := by
      by_cases hkAnchor : i.val - 1 ≤ D.anchor
      · exact C.source_to_anchor (i.val - 1) hkAnchor hpreviousEven
      · have hanchorK : D.anchor ≤ i.val - 1 :=
        ...
        have hdistance : Even (i.val - 1 - D.anchor) := by
          rcases hanchorEven with ⟨e, he⟩
          exact ⟨d - e, by omega⟩
        exact C.source_to_right (i.val - 1) hanchorK hright.le
          hdistance
    have htargetGap := lemma69_v_typeI_even_entry_gap_two
    ...
    have hbParityRaw := a.lemma72_typeI_target_after_of_canonical
      b D C hfirst (i.val + 2) (by omega) (by
        have hr := C.right_le_last
        omega)
