...
Theorem 2.5(ii) at `j`. -/
theorem he2022ClassicLemma34_general {m n : Nat}
    ...
    (hjEven : Even j.val)
    (hRj : a.order ⟨j.val - 1,
      Nat.lt_of_le_of_lt (Nat.sub_le _ _) j.lt_large⟩ = 0)
    (hRjOne : a.order ⟨j.val, j.lt_large⟩ = 0)
    (halphaJ : a.alphaValue ⟨j.val - 1,
      (by have h := j.lt_large; omega)⟩ = 1)
    (hfield :
  ...
  have hjGapBound : j.val - 1 < m + 1 := by
    have h := j.lt_large
    omega
  let sourceGap : Fin (m + 1) := ⟨j.val - 1, hjGapBound⟩
  ...
  · have hcomparisonEven : Even (ordUnit K
    ...
    have hAlphaOne : a.representationAlpha b j ≤ (1 : WithTop ℚ) := by
      rcases hfield with heOne | ⟨hjNextTwo, heLarge⟩
...
/-- Endpoint form of Lemma 3.4 in the ramification-one branch.  Unlike the
large-ramification branch, this case does not require a source coefficient
after paper index `j+1`; this is the form used at the terminal index in the
author-corrected v5 Lemma 7.1(ii). -/
theorem he2022ClassicLemma34_ramificationOne {m n : Nat}
    (a : GoodBONG q L (m + 2)) (b : GoodBONG r M (n + 2))
    (hAClassic : Lattice.IsClassicIntegral q L)
    (hBClassic : Lattice.IsClassicIntegral r M)
    (j : RepresentationIndex (m + 2) (n + 2))
    ...
    a.HeClassicDefectConditionAt b j := by
  exact a.he2022ClassicLemma34_general b hAClassic hBClassic j hjEven
    hRj hRjOne halphaJ (Or.inl heOne)

/-- Backwards-compatible form of Lemma 3.4 used away from the terminal
source index. -/
        ...
        (1 < ramificationIndex K ∧
          a.truncatedPrefixDefect a ((-1) ^ ((j.val + 2) / 2))
            ...
            ((((1 - a.order ⟨j.val + 1, hjNextTwo⟩ : Int) : ℚ) :
              WithTop ℚ)))) :
    a.HeClassicDefectConditionAt b j := by
  apply a.he2022ClassicLemma34_general b hAClassic hBClassic j hjEven
    hRj hRjOne halphaJ
  rcases hfield with heOne | heLarge
  · exact Or.inl heOne
  · exact Or.inr ⟨hjNextTwo, heLarge⟩
