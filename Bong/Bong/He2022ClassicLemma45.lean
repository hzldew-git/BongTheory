...
/-- A direct terminal upper bound implies the complete condition `J2_E(n)`.

This separates the arithmetic heart of Lemma 4.5 from its particular testing
rows.  It is useful for repairing downstream arguments without routing them
through the false literal Lemma 7.1(ii). -/
theorem he2022ClassicLemma45_j2_of_terminalUpper
    [QuadraticDefectLaws K] [HilbertSymbolLaws K]
    [DyadicDiscriminantClassLaws K]
    {m : Nat} (t : Nat) (a : GoodBONG q L (m + 4))
    (hSource : 2 * t + 4 <= m + 4)
  ...
  let D : WithTop ℚ :=
    a.truncatedPrefixDefect a ((-1) ^ (t + 2)) 0 (2 * t + 4)
  have hDUpper : D <= ((((1 - R : Int) : ℚ) : WithTop ℚ)) :=
    truncatedPrefixDefect_le_one_sub_order_of_add_le R D (by
      simpa only [R, D] using hUpper)
  have hNextAlpha := a.he2022ClassicLemma45_nextAlpha t hSource hJ1 hUpper
  have hFirst : a.order (0 : Fin (m + 4)) = 0 := by
    let first : Fin (2 * t + 3) := ⟨0, by omega⟩
    have h := hJ1.1 first
    have hIndex : (⟨first.val, by omega⟩ : Fin (m + 4)) = 0 := Fin.ext rfl
    rw [hIndex] at h
    exact h
  have hRNonnegative : 0 <= R := by
    exact (a.he2022ClassicProposition24 hAClassic).nonnegativeOfFirstZero
      hFirst ⟨2 * t + 3, by omega⟩
  have hDLower : ((((1 - R : Int) : ℚ) : WithTop ℚ)) <= D := by
  ...
  · have hNext : 2 * t + 2 + 1 = 2 * t + 3 := by omega
    ...
    simpa only [hNext, hLength, hExponent] using hEquality
  · exact a.he2022ClassicLemma45_binaryRank_of_ambient_and_equality t
      hAmbient hSource hEquality

/-- Over a ramified dyadic field with `e > 1`, `J2'_E(n)` already supplies
the terminal upper bound, so no Lemma 4.5 test row is needed to obtain
`J2_E(n)`. -/
theorem he2022ClassicLemma45_j2_of_j2Prime_of_ramification_gt_one
    [QuadraticDefectLaws K] [HilbertSymbolLaws K]
    [DyadicDiscriminantClassLaws K]
    {m : Nat} (t : Nat) (a : GoodBONG q L (m + 4))
    (hSource : 2 * t + 4 <= m + 4)
    ...
    a.HeClassicJ2E (2 * t + 2) (by omega) := by
  apply a.he2022ClassicLemma45_j2_of_terminalUpper t hSource hAClassic
    hAmbient hJ1
  have hUpper := hJ2Prime heLarge
  have hHalfIndex : (2 * t + 2 + 2) / 2 = t + 2 := by omega
  have hOrderIndex : 2 * t + 2 + 1 = 2 * t + 3 := by omega
  have hLength : 2 * t + 2 + 2 = 2 * t + 4 := by omega
  simpa only [hHalfIndex, hOrderIndex, hLength] using hUpper
