...
/-- Author-corrected v5, odd-rank half of Theorem 1.3: the literal published
family tests classic universality, and deleting any displayed row destroys
that property. -/
theorem he2022ClassicTheorem13_odd_literalMinimal_v5
    [QuadraticDefectLaws K] [HilbertSymbolLaws K]
    [DyadicDiscriminantClassLaws K]
    {I : Type u} [Fintype I] (U : I → Kˣ)
    (hU : IsHeHuCompleteUnitRepresentativeSystem (K := K) U)
    (omegaData : HeClassicOmegaData (K := K)) (pairs : Nat) :
    Lattice.QuadraticLatticeModel.IsLiteralMinimalClassicUniversalityTestingFamily
      (HeClassicPublishedOddTestingIndex.model
        (K := K) U hU omegaData pairs) (2 * pairs + 3) := by
  constructor
  · intro X hXClassic hAll
    exact Lattice.QuadraticLatticeModel.all_publishedOdd_implies_classicUniversal_v5_auto
        (K := K) U hU omegaData pairs X hXClassic hAll
  · intro i
    exact he2022ClassicLemma711_publishedOdd_deletionWitness
