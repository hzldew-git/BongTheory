...
/-!
# Beli (2019), Lemma 7.9(ii), case 3: type-II interior assembly

The source gap is at most `2e`.  At equality the half-gap candidate closes
the beta estimate; below `2e`, the alpha shift and the three shifted
candidate comparisons close it.  The capped-prefix assembly then proves
condition 2.1(ii).
-/
...
-- This is the complete scalar estimate in the strict type-II left interior.
/-- The representation alpha is bounded by the preceding type-II target
alpha at every strict even left-interior boundary. -/
theorem beli2019Lemma79_typeII_even_left_beta
  ...
  · have hsmall : b.orderGap ⟨i.val - 1, by
      ...
      lt_of_not_ge hlarge
    have halpha := beli2019Lemma79_typeII_even_left_alphaShift
    ...
    have hsecondary : ∀
        ...
        b.representationSecondaryDefect c i hi' ≤
          a.representationSecondaryDefect c i hi' +
            ((2 : ℚ) : WithTop ℚ) := by
      intro hi'
      exact beli2019Lemma79_typeII_even_left_secondary
        a b c D hfirst hdefectAB i hi' hiEven hfarLeft
    exact lemma79_even_beta_bound_of_candidate_shifts
      a b c hdefectAC i halpha hhalf hprimary hsecondary

/-- Lemma 7.9(ii), case 3, at every strict even type-II left-interior
boundary. -/
theorem beli2019Lemma79_ii_typeII_even_left_interior
    [Beli2006AlphaLaws.{u, v} K]
    [Beli2009AlphaParityLaws.{u, v} K]
    (a : GoodBONG q L (n + 2)) (b : GoodBONG q M (n + 2))
    (c : GoodBONG q N (n + 2))
  ...
  have hbeta := beli2019Lemma79_typeII_even_left_beta
    a b c D hfirst hdefectAB hdefectAC horderBC
