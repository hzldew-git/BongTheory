...
-/
import Bong.Lattice.Omeara9328QuaternaryCoefficientShiftReplacement
import Bong.Lattice.Omeara9328ScaledCoefficientShift

/-!
# Installing scaled O'Meara 93:19 in a quaternary Jordan head

O'Meara normalizes the first Jordan scale to one during the coefficient
calculation.  This file installs the rescaled identity back into the original
Jordan decomposition.  Thus neither the replacement geometry nor the
norm-group containment needs an additional local-law parameter.
-/
...
variable {K : Type u} [Field K] [CharZero K] [ValuativeRel K]
  ...
  {r : QuadraticSpace K Y} {N : Lattice K Y} {s a : Kˣ}

/-- Scaled version of `quaternaryCoefficientShiftJordanReplacement`.
The normalized 93:19 calculation has scale `s`; after multiplying the form
by `a`, its complement is modular at `a * s`. -/
noncomputable def scaledQuaternaryCoefficientShiftJordanReplacement
    (J : JordanDecomposition q L (n + 2))
    (hJ : J.IsSaturated)
    (E : Omeara9319ExchangeSetup r N s)
    (hN : IsModular r N s) (hrank : 3 ≤ finrank K Y)
    (headIso : Isometry
      ((p.rescaleUnit a).orthogonalSum (E.oldPlane.rescaleUnit a))
      (J.component 0).space
      (product A (hyperbolicPlaneLattice (K := K)))
      (J.component 0).lattice)
    (tailIso : Isometry (r.rescaleUnit a) (J.component 1).space
      N (J.component 1).lattice)
    (hnewHeadModular : IsModular
      ((p.rescaleUnit a).orthogonalSum (E.newPlane.rescaleUnit a))
      (product A (hyperbolicPlaneLattice (K := K)))
      (J.scaleGenerator 0))
  ...
  let complementRescaled :=
    (D.splitting.decomposition.component 1).space.rescaleUnit a
  let expose : Isometry
      (((p.rescaleUnit a).orthogonalSum (E.oldPlane.rescaleUnit a)).orthogonalSum
        (r.rescaleUnit a))
  ...
  let leftIdentity := Isometry.refl (p.rescaleUnit a) A
  let reassociateNew : Isometry
      (((p.rescaleUnit a).orthogonalSum (E.newPlane.rescaleUnit a)).orthogonalSum
        complementRescaled)
      ((p.rescaleUnit a).orthogonalSum
        ((E.newPlane.rescaleUnit a).orthogonalSum complementRescaled))
  ...
  let undoShift : Isometry
      ((p.rescaleUnit a).orthogonalSum
        ((E.newPlane.rescaleUnit a).orthogonalSum complementRescaled))
      ((p.rescaleUnit a).orthogonalSum
        ((E.oldPlane.rescaleUnit a).orthogonalSum (r.rescaleUnit a)))
      (product A
        (product (hyperbolicPlaneLattice (K := K))
          (D.splitting.decomposition.component 1).lattice))
      (product A (product (hyperbolicPlaneLattice (K := K)) N)) :=
    leftIdentity.orthogonalProductBasic
      (E.shiftedRescale hN hrank a).symm
  ...
  have htailContains : normGroupSet (J.component 1).space
      ...
      normGroupSet complementRescaled
        (D.splitting.decomposition.component 1).lattice := by
    intro z hz
    apply E.normGroupSet_subset_complement_rescale hN hrank a
